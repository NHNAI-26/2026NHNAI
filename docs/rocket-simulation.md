...
그래서 상승감을 맡는 것은 지구가 아니라 **별**이다. 지구는 far clip 부등식(아래 림 795 < 1000) 때문에
카메라에 매달아 두고 지름 850 → 657 로 줄어드는 크기 변화만 담당한다 — 축 위에 얹힌 구라 평행이동
시차를 만들 수 없다.
...
별은 텍스처 없는 파티클 점 800개이고(Sphere 껍질 반지름 400, `radiusThickness 0`, `simulationSpace = Local`),
알파는 고도 곡선이 올린다. 위치는 매 프레임 **XZ 만** 카메라를 따라간다 — Y 는 `spaceKm` 위에서
`starParallax`(0~1, 기본 1) 만큼 놓아줘 껍질이 월드에 그대로 선다(`SkyEnvironment.StarLagUnits`).

Y 까지 못박아 두었을 때가 문제였다. `spaceKm` 위에서는 수면도 구름도 렌더러가 꺼지고, 스카이박스는
무한원이라 평행이동에 반응하지 않으며, 지구도 카메라에 매달려 있다 — **화면 안에 카메라 대비 움직이는
것이 하나도 남지 않아** 우주에서 상승 중인데도 멈춰 있는 것처럼 보였다.

껍질을 놓으면 우주 진입 시점 각속도가 `55 / 400 ≈ 7.9°/s` 다(진입 수직속도 `√(2·9.81·154) ≈ 55 유닛/s`).
별 하나가 60° 화각을 약 8초에 가로지른다. 정점에서 0 으로 잦아드는 것은 연출 실패가 아니라 **정직한
감속**이다 — 관성 상승이라 실제로 멈추는 중이고, 낙하로 뒤집히면 별은 반대로 위로 흐른다.

상한은 껍질 반지름이 정한다. 정점(월드 y 434, 우주 진입 280)까지 이탈이 **154 / 400 = 38 %** 라 카메라가
껍질 안에 남는다. `starParallax` 를 1 위로 열거나 껍질 반지름을 줄이면 카메라가 위쪽 껍질을 뚫고 나가
천정이 빈다. 경계에서는 이탈이 0 이라 우주 진입 순간 껍질이 튀지 않는다.
