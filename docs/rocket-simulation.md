...
저전력 검증은 목표 구역 체류 판정은 그대로 두고 설치 엔진을 6개로 제한한다. `LaunchMissionController` 가
`RocketBuilder` 에 미션별 부착 상한을 설정한다. 여섯 개가 붙은 뒤 프리셋을 다시 끌면 새 엔진을 만들지
않는다. 이미 붙은 엔진의 이동과 삭제는 계속 가능하고, 다른 미션과 단독 테스트에는 상한을 적용하지 않는다.
