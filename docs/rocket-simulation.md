...
`SkyEnvironment` 가 강제로 켜는 안개(`fogDensity` 0.002 → 0)가 발사대 근처 점을 배경색 쪽으로 씻어낸다.
...
미션 컨트롤 모드의 배치는 `RocketDesignUI.prefab` 이 소유한다. 기준 해상도 1280×720, 아래 값들은 프리팹
저작값이고 코드는 그것을 읽을 뿐이다 — 상수로 복사해 두면 프리팹에서 고쳐도 런타임에 되돌아간다.

`UpdateViewportRect` 는 도크(`PresetPanel`/`FlightInfoPanel`)가 꺼졌을 때만 `Viewport` 와 `StageStrip` 의
왼쪽 오프셋을 0 으로 접고, 다시 켜지면 Bind 시점에 기억해 둔 프리팹 값으로 되돌린다. 그러니 도크 폭을
바꾸려면 프리팹에서 `PresetPanel` 폭과 `Viewport`·`StageStrip` 의 왼쪽 오프셋을 같이 고치면 된다.
코드 상수 `DockWidth` 200 은 이제 테스터 화면(`BuildInterface`) 전용이다.
...
- **안 쓰는 단계는 조건이 우연히 맞아도 세지 않는다.** 고도 미션은 목표 구역 안을 지나가더라도 그것으로
  진행했다고 보지 않는다. 상승 기준 고도도 미션의 목표 고도를 따른다(낮은 고도 100, 높은 고도 250, 그 외 200).

저전력 검증은 연소 제한 초과를 실격으로 처리하지 않는다. `LaunchMissionController` 가 로켓에 엔진 수와
무관한 실제 연소 시간 8초를 설정하고, `Rocket` 은 제한에 닿는 물리 스텝에서 남은 일부만 태운 뒤 모든
엔진을 자동 차단한다. 클램프 대기 중에는 연료를 태우지 않아 이 시간도 늘지 않는다. `Rigidbody` 는 멈추지
않으므로 로켓은 관성 비행을 계속하고 목표 구역 체류도 정상 판정된다.
...
지오메트리도 두 번 그려진다. 그래서 얹는 동안 `cullingMask = 0` + `clearFlags = SolidColor` +
`backgroundColor = Color.black` 로 바꾸고 내릴 때 세 값을 모두 되돌린다.

지우는 색을 코드가 쥐는 이유: 씬에 저작된 색(`01_Main` 은 유니티 기본 파랑)을 그대로 쓰면 CRT 화면
안에서 UI 뒤 — 뷰포트 사각형 밖 — 이 그 파랑으로 채워진다. 씬에서 카메라 색만 검게 고치면 씬 병합이나
재저장 한 번에 되살아나므로 씬이 아니라 여기서 정한다.
...
1. **상승** — 가운데가 뚫린 16:9 베젤 메시(`Resources/displayFrame`)가 화면 아래에서 올라와 화면을
   ...
   상승을 시작할 때 `woosh2` 효과음을 1.3배속(`pitch = 1.3`)으로 2D 단발 재생한다. 베젤이 없으면 재생하지 않는다.
   덮개는 이때 알파 0 이 되지만 살아 있는다. 전원 값이 1 이라 화면은 어차피 검고, `raycastTarget` 이
   남아 있어야 연출 중 클릭이 뒤로 새지 않는다.
2. **확대** — 베젤을 `ZoomScale`(1.8, 눈으로 맞추는 아트 값) 배로 키워 화면 밖으로 밀어낸다. 이 구간
   내내 화면은 아직 꺼져 있어 뚫린 가운데는 검다. 확대가 끝나면 베젤은 사라지고, 파괴하지 않고
   카메라만 꺼 둔다 — 퇴장에서 역방향으로 다시 쓴다.
3. **점등** — 전원 값 `1 → 0`. 베젤이 이미 화면 밖이라 브라운관이 켜지는 그림이 가려지는 곳 없이
   풀스크린으로 보인다. 점등을 맨 뒤에 두는 이유가 이것이다 — 베젤 안쪽 사각형에서 켜면 전원 연출의
   가장자리(곡률·수축)가 테두리에 잘려 브라운관이 아니라 창이 밝아지는 것처럼 읽힌다.
   확대가 조금이라도 모자라 베젤 테두리가 남으면 그것이 점등 첫 프레임에 그대로 잡히므로,
   `ZoomScale` 은 이 순서에서 더 민감한 값이다.

퇴장은 정확한 역순이다: 소등 → 축소 → 베젤 하강. 내릴 때는 오버슛을 쓰지 않는다(smoothstep). 그다음
