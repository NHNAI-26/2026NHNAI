...
저전력 검증은 연소 예산 초과를 실격으로 처리하지 않는다. `LaunchMissionController` 가 로켓에 전체 엔진
누적 연소 예산 8초를 설정하고, `Rocket` 은 예산에 닿는 물리 스텝에서 남은 일부만 태운 뒤 모든 엔진을
자동 차단한다. `Rigidbody` 는 멈추지 않으므로 로켓은 관성 비행을 계속하고 목표 구역 체류도 정상 판정된다.
