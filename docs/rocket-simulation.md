...
각도 스냅은 두지 않으므로(UD-011) 엔진을 뒤집어 로켓을 땅으로 밀어내는 배치까지 가능하다.
...
- **회전 자세는 정렬 대상이 아니다.** 기즈모가 축을 구속할 뿐 각도 스냅(15° 등)은 없다. 추력이
  `engine.transform.up`을 따르므로 위치가 대칭이어도 자세가 다르면 토크는 상쇄되지 않는다.
