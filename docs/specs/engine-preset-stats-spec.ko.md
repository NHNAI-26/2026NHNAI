...
- **In scope:** 물리 단위 SO 데이터 모델, 10슬롯 제한, 열 모델, 설계 단계 값 소비, 에디터 테스트 채우기 툴.
- **Out of scope:** 미니게임 구현, 런타임 프리셋 편집 UI, 런타임 슬롯 저장, GDD 06 공식 재설계.
...
| 런타임 프리셋 편집 UI | UD-005 | 데이터 + 읽기 + 테스트 툴까지 |
...
3. 프리셋을 최대 10개 슬롯 목록에 등록한다.
4. 플레이어가 로켓 설계 단계에 진입한다.
5. 설계 단계가 슬롯을 읽어 선택 가능한 엔진을 보여준다.
