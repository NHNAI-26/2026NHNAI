...
`RocketPart`가 `thrust = 1200f`, `fuel = 100f`, `burnRate = 20f`를 하드코딩한다(SF-005). "엔진마다 다르다"가 GDD에는 있으나 데이터에는 없다. 옛 정규화 체계는 `최대 출력 − 냉각 능력`처럼 단위가 다른 값을 직접 비교했다(SF-001, SF-009, SF-017). 사용자는 정규화 대신 물리 단위 직접 저장을 선택했다(UD-010). 열 모델은 `Overheat` 종료에 실제 시뮬레이션 근거를 준다(SF-018).
...
열 밸런스를 문서화된 값으로 되돌렸다: `EngineStatsSO.HeatPerNewton = 0.05`.
연구값 런타임 배율, 엔진별 온도 누적, 공통 `300 °C` 임계값, 과열 시 발사 전체 실패 규칙은 바꾸지 않았다.
`56 / 98 / 87 / 41` 연구 스탯 회귀 테스트를 추가해 높은 냉각 프리셋이 연료 소진 전에 폭발하지 않도록 잠갔다.
