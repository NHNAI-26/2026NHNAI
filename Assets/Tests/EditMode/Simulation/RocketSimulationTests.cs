...
namespace Simulation.Tests
{
    public sealed class RocketSimulationTests
    {
        ...
        [Test]
        public void RocketBuilder_AttachmentLimitBlocksTheSeventhNewEngine()
        {
            var rocketGo = Track(new GameObject("limited rocket"));
            var rocket = rocketGo.AddComponent<Rocket>();
            var builder = Track(new GameObject("limited builder")).AddComponent<RocketBuilder>();
            SetField(builder, "rocket", rocket);
            builder.SetEngineAttachmentLimit(6);

            for (int i = 0; i < 6; i++)
            {
                Assert.That(builder.CanAttachNewEngine, Is.True);
                rocket.Attach(CreateEngine(Stats(100f, 1000f, BaselineOutput, 100f)),
                    new Vector3(i, 0f, 0f));
            }

            Assert.That(builder.CanAttachNewEngine, Is.False);
            builder.BeginPresetDrag(Stats(100f, 1000f, BaselineOutput, 100f), Vector2.zero);
            Assert.That(builder.Selected, Is.Null, "6개 뒤에는 7번째 엔진 드래그를 시작하지 않아야 한다.");
        }
        ...
        [TestCase(100f, 60f, 1200f)]
        [TestCase(220f, 62f, 1150f)]
        [TestCase(120f, 80f, 2400f)]
        [TestCase(110f, 68f, 1300f)]
        public void RuntimeBridge_HighCoolingResearchSurvivesUntilFuelRunsOut(
            float baseFuel,
            float baseCooling,
            float baseOutput)
        {
            var state = new EnginePresetState
            {
                FuelCapacity = 56,
                Cooling = 98,
                MaxOutput = 87,
                IgnitionReliability = 100,
            };
            EngineStatsSO runtime = Track(ResearchEnginePresetRuntimeBridge.BuildRuntimePreset(
                0,
                Stats(baseFuel, baseCooling, baseOutput, 100f),
                state));
            RocketPart part = CreateEngine(runtime);
            part.Prepare(new DeterministicRng());

            for (int i = 1; i <= 2000 && part.HasFuel && !part.Overheated; i++)
                part.Tick(0.02f, Rocket.RampFactor(i * 0.02f, 1.2f));

            Assert.IsFalse(part.Overheated, "냉각 98 연구 프리셋은 연료 소진 전 과열 폭발하지 않아야 한다.");
            Assert.IsFalse(part.HasFuel);
        }
        ...
        [TestCase(90f, 55f, 1800f, false)]
        [TestCase(120f, 80f, 2400f, false)]
        [TestCase(90f, 100f, 1800f, false)]
        [TestCase(200f, 10f, 2400f, true)]
